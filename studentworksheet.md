...
| 3 | vks-01 cluster IP | `_._._._` | Run: `kubectl config view --kubeconfig ~/.kube/config \| grep server` — or VCFA UI → dev-xxxxx → Kubernetes → vks-01 |
...
| 7 | vks-01 cluster IP | `_._._._` | Run: `kubectl config view --kubeconfig ~/Downloads/vks-01-kubeconfig.yaml \| grep server` — or VCFA UI → test-xxxxx → Kubernetes → vks-01 |
| 8 | DB_HOST (oc-mysql VM LB) | `_._._._` | Run: `vcf context use supervisor:test-xxxxx` → `kubectl get service` — or VCFA UI → test-xxxxx → Virtual Machine → oc-mysql → Network Service |
